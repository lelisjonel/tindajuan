  ...
  return (
    <div className="space-y-5">
      ...
      <section className="tj-card p-4 sm:p-5">
        <div className="mb-4"><h2 className="text-lg font-black text-[var(--primary-dark)]">Load / Bills Payment</h2><p className="text-sm text-[var(--muted)]">Face value = service amount. Wallet deduction = actual GCash/Maya cost. Customer pays = amount you collect.</p><p className="mt-2 rounded-xl bg-green-50 px-3 py-2 text-xs font-bold text-green-900">Customer price includes your service fee.</p></div>
        <div className="grid gap-3 lg:grid-cols-[180px_180px_1fr]">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {walletOutType === "load" ? <><label className="block text-sm font-bold text-[var(--primary-dark)]">Load product<input className="tj-input mt-2" onChange={(event) => setProductName(event.target.value)} placeholder="e.g. Go Unli 50" value={productName} /></label><label className="block text-sm font-bold text-[var(--primary-dark)]">Mobile Number<input className="tj-input mt-2" inputMode="tel" onChange={(event) => setMobileNumber(event.target.value)} placeholder="09XXXXXXXXX" value={mobileNumber} /></label></> : <><label className="block text-sm font-bold text-[var(--primary-dark)]">Biller<input className="tj-input mt-2" onChange={(event) => setBiller(event.target.value)} placeholder="e.g. Meralco" value={biller} /></label><label className="block text-sm font-bold text-[var(--primary-dark)]">Account Reference<input className="tj-input mt-2" onChange={(event) => setAccountReference(event.target.value)} placeholder="Account / subscriber number" value={accountReference} /></label></>}
